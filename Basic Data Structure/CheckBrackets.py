    ...
    for i, c in enumerate(str, start=1):
        if c in open_brackets:
            ...
            if not stack: #If the stack is empty, we don't have an opening bracket for our closing bracket, so we return the index of the first closing bracket.
                return i
            if open_brackets[stack[-1][0]] != c: #If the closing bracket doesn't match the opening bracket, we return the first unmatched closing bracket.
                return i
            stack.pop()
    if stack:
        return stack[-1][1]    #If the stack doesn't have any closing bracket, we return the index of the first opening bracket.
    return "Success"
